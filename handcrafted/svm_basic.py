...
def evaluateModel(tclf, X_all, y_all):
    ...
    for jc in mDataCats:
        ...
        for mtarget in mTargets:
            for ty in types:
                ...
                X_train, y_train = getDataset(mpath)
                rlist = evaluateModel(tclf, X_train, y_train)
                tdf = pd.DataFrame(rlist, columns=mcols)
                mdf = mdf.append(tdf, ignore_index=True)
                print ("Done : " + mpath)
        mpath = "../result/handcrafted/score/" + jt + "/" + jc + "/" + "rskf_svm.csv"
